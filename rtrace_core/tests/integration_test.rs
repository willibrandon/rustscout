...
fn test_find_todo_comments() {
    ...
    assert_eq!(result.files_with_matches, 4, "Expected to find TODOs in 4 files");

    // Verify each TODO was found
    let todos: Vec<_> = result.file_results
        .iter()
...
fn test_find_rust_functions() {
    ...
    assert_eq!(result.total_matches, 5, "Expected to find 5 function definitions");

    // Verify each function was found
    let functions: Vec<_> = result.file_results
        .iter()
        .flat_map(|fr| fr.matches.iter().map(|m| m.line_content.clone()))
        .collect();
    ...
    assert!(functions.iter().any(|f| f.contains("fn add(a: i32, b: i32)")));
    assert!(functions.iter().any(|f| f.contains("fn test_add()")));
    assert!(functions.iter().any(|f| f.contains("fn test_integration()")));

    // Verify no files from target/ were searched
    for file_result in &result.file_results {
        assert!(!file_result.path.to_string_lossy().contains("target/"),
            "Found matches in ignored directory: {}", file_result.path.display());
    }
}
...
fn test_ignore_patterns() {
    ...
    for file_result in &result.file_results {
        ...
        assert!(!path.contains("target/"), "Should not search files in target/");
...
fn test_empty_and_invalid_patterns() {
    ...
    let result = search::search(&config).unwrap();
    assert_eq!(result.total_matches, 0, "Empty pattern should find no matches");
...
fn test_large_file_search() {
    ...
    let result = search::search(&config).unwrap();
    assert_eq!(result.total_matches, 1000, "Should find 1000 TODOs in large file");
